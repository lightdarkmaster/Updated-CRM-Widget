    ...
    try {
      ...
      if (resp && resp.data && resp.data.length > 0) {
        const filteredData = resp.data.filter((lead) => {
          const leadSource =
            lead.Lead_Source || lead["Lead Source"] || lead.lead_source || "";

          const matchesLeadSource = leadSources.includes(leadSource);

          return matchesLeadSource;
        });
...
function groupLeadsByMonthWeek(leads, year) {
  ...
  leads.forEach((lead) => {
    ...
    try {
      ...
      if (isNaN(createdDate.getTime()) || createdDate.getFullYear() !== year)
        return;
...
function renderTable(monthlyWeeklyCounts, year, totalFiltered) {
  ...
  thead.innerHTML = `<tr style="background:#4CAF50;color:white;">
        ...
        ${months
          .map(
            (m) =>
              `<th style="padding:12px;border:1px solid #ddd;text-align:center;">${m}</th>`
          )
          .join("")}
    </tr>`;
...
ZOHO.embeddedApp.on("PageLoad", async () => {
  ...
  try {
    ...
    const monthlyWeeklyCounts = groupLeadsByMonthWeek(
      filteredLeads,
      targetYear
    );
