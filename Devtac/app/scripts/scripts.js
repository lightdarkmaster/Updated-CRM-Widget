...
// Returns week number (1–5) in the month
...
async function fetchFilteredLeadsCOQL() {
    ...
    const leadSources = [
        "Devtac Blog", "Devtac Chat", "Devtac Facebook", "Devtac LinkedIn",
        "Devtac Newsletter", "Devtac Twitter", "DEVTAC Website", "Direct Email Inquiry",
        "Devtac Direct Call / Text", "Employee Referral", "Google AdWords",
        "Devtac Webinar", "Self-Generated", "WebSite Visit", "Chat",
        "Others", "Employee Referral", "Customer Referral"
    ];
    ...
    const query = `
        SELECT Created_Time, Lead_Source
        FROM Leads
...
function groupLeadsByMonthWeek(leads, year) {
    ...
    leads.forEach(lead => {
        const dateValue = lead.Created_Time;
        if (!dateValue) return;
...
ZOHO.embeddedApp.on("PageLoad", async () => {
    ...
    document.body.innerHTML = `
        <div id="loadingDiv" style="text-align:center;padding:40px;background:#e3f2fd;border-radius:8px;margin:20px;">
            ...
            <p>Applying Lead Source filters...</p>
        </div>
    ...
    try {
        ...
        if (!leads.length) {
            document.body.innerHTML = `<div style="text-align:center;padding:40px;background:#fff3cd;border-radius:8px;margin:20px;border-left:5px solid #ffc107;">
                ...
                <p>No leads found matching the specified Lead Source criteria.</p>
            </div>`;
            return;
        }
